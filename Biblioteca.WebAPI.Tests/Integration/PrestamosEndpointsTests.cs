...
public class PrestamosEndpointsTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task Admin_Can_Create_And_Return_Prestamo()
    {
        ...
        var respAutor = await client.PostAsync("/api/autores", new StringContent(JsonSerializer.Serialize(new CrearAutorDto{ Nombre = "A", Apellido = "B"},_json), Encoding.UTF8, "application/json"));
        ...
        var respGenero = await client.PostAsync("/api/generos", new StringContent(JsonSerializer.Serialize(new CrearGeneroDto{ Nombre = "G"},_json), Encoding.UTF8, "application/json"));
        ...
        var respEditorial = await client.PostAsync("/api/editoriales", new StringContent(JsonSerializer.Serialize(new CrearEditorialDto{ Nombre = "E"},_json), Encoding.UTF8, "application/json"));
        ...
        var respLibro = await client.PostAsync("/api/libros", new StringContent(JsonSerializer.Serialize(new CrearLibroDto{ Titulo = "T", ISBN = isbn, AutorId = autor.Id, GeneroId = genero.Id, EditorialId = editorial.Id, Estado = "disponible"},_json), Encoding.UTF8, "application/json"));
        ...
        var respPersona = await client.PostAsync("/api/personas", new StringContent(JsonSerializer.Serialize(new CrearPersonaDto{ Nombre = "Nom", Apellido = "Ape", Dni = Guid.NewGuid().ToString("N").Substring(0,8), Email = $"e{Guid.NewGuid():N}@e.com"},_json), Encoding.UTF8, "application/json"));
        ...
        var respUsuario = await client.PostAsync("/api/usuarios", new StringContent(JsonSerializer.Serialize(new CrearUsuarioDto{ NombreUsuario = $"socio_{Guid.NewGuid():N}", Clave = "socio123", Rol = "socio", PersonaId = persona.Id},_json), Encoding.UTF8, "application/json"));
